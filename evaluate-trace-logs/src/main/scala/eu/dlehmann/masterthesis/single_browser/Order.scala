...
object Order extends App {
  ...
  case class Parameters(browser: String, bpIds: Boolean, cols: Boolean, breakpointsPerLine: Double, testcase: String) extends DataClass


  object Parameters {
    private val filenameMatcher = """params(-bpids)?(-cols)?-s([0-9]+)-bp([0-9\.]+)-a([0-9\.]+)/(.*)/(.*)-(firefox|chrome).trace.log""".r

    def fromFilename(filename: String): Option[Parameters] = filename match {
      case filenameMatcher(bpids, cols, seed, breakpointsPerLine, actionsCount, testDir, testcase, browser) =>
        Some(Parameters(browser, bpids != null, cols != null, breakpointsPerLine.toDouble, (testDir + '-' + testcase).replace("pta-warm-up-quizzes","js-puzzle")))
      case _ => None
    }

  }

  val logsDir = Paths.get("/home/daniel/Documents/masterthesis/implementation/debugger-common-api/log/thesis/single-debugger/order")
  ...
  val experiments: Map[Parameters, Outputs] = logsDir.recursiveList
    ...
    .mapValues {
      case filesGroup if filesGroup.size > 1 =>
        // generate all possible pairings of seeds
