...
determ = read.csv("determinism/results.csv")
levels(determ$restart) <- c("No Restart between Runs", "Restart between Runs")
levels(determ$browser) <- c("Chromium", "Firefox")

cairo_pdf("determinism-effectiveness.pdf", family="Myriad Pro", width=9, height=5)
print(
  ggplot(determ, aes(x = testcase, y = linesDiffByLinesSum, fill = browser))
  # + geom_point()
  + facet_grid(restart~.)
  ...
  + labs(y = "Normalized Difference Rate", x = "Test Program", fill = "Browser")
  #+ theme(legend.position="bottom")
)
dev.off()


order = read.csv("order/results.csv")
levels(order$browser) <- c("Chromium", "Firefox")

cairo_pdf("order-effectiveness.pdf", family="Myriad Pro", width=9, height=5)
print(
  ggplot(order, aes(x = testcase, y = linesDiffByLinesSum, fill = browser))
  # + geom_point()
  + scale_fill_manual(values = c("#0266C8", "#E66000"))
  + theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
inverse = read.csv("inverse/results.csv")
levels(inverse$browser) <- c("Chromium", "Firefox")

cairo_pdf("inverse-effectiveness.pdf", family="Myriad Pro", width=9, height=5)
print(
  ggplot(inverse[inverse$breakpointsPerLine == 1 & inverse$removeProbability == 0.5,], aes(x = testcase, y = linesDiffByLinesSum, fill = browser))
  # + geom_point()
  + scale_fill_manual(values = c("#0266C8", "#E66000"))
  + theme(axis.text.x = element_text(angle = 45, hjust = 1))
  + geom_bar(stat = "identity", width = .5)
...
transform = read.csv("transform/results.csv")
levels(transform$browser) <- c("Chromium", "Firefox")
levels(transform$transformation) <- c("Hoist Functions", "Hoist & Randomize Functionms", "Hoist Vars", "Rand Funs", "ESmangle", "UglifyJS2")

cairo_pdf("transform-effectiveness.pdf", family="Myriad Pro", width=9, height=5)
print(
  ggplot(transform[transform$transformation == "Rand Funs" | transform$transformation == "Hoist Vars" | transform$transformation == "ESmangle" | transform$transformation == "UglifyJS2",], aes(x = testcase, y = linesDiffByLinesSum, fill = browser))
  # + geom_point()
  + scale_fill_manual(values = c("#0266C8", "#E66000"))
  + theme(axis.text.x = element_text(angle = 45, hjust = 1))
  ...
  + labs(y = "Normalized Difference Rate", x = "Test Program", fill = "Browser")
  #+ theme(legend.position="bottom")
  + facet_wrap(~transformation, nrow = 4, strip.position="right")
  + theme(strip.background=element_rect(fill="white"))
