...
function Triangle(p1, p2, p3) {
    ...
    if (Math.abs(normal[0]) > Math.abs(normal[1])) if (Math.abs(normal[0]) > Math.abs(normal[2])) this.axis = 0; else this.axis = 2; else if (Math.abs(normal[1]) > Math.abs(normal[2])) this.axis = 1; else this.axis = 2;
    var u = (this.axis + 1) % 3;
    var v = (this.axis + 2) % 3;
    var u1 = edge1[u];
    var v1 = edge1[v];
...
Scene.prototype.intersect = function(origin, dir, near, far) {
    ...
    var reflected = null;
    if (colour.reflection > .001) {
        var reflection = addVector(scale(normal, -2 * dot(dir, normal)), dir);
        reflected = this.intersect(hit, reflection, 1e-4, 1e6);
        if (colour.reflection >= .999999) return reflected;
    }
...
function raytraceScene() {
    ...
    var green = createVector(0, .4, 0);
    var grey = createVector(.4, .4, .4);
    grey.reflection = 1;
    var floorShader = function(tri, pos, view) {
        var x = (pos[0] / 32 % 2 + 2) % 2;
        var z = ((pos[2] / 32 + .3) % 2 + 2) % 2;
        if (x < 1 != z < 1) {
            return grey;
        } else return green;
    };
    var ffl = createVector(-1e3, -30, -1e3);
    var ffr = createVector(1e3, -30, -1e3);
    var fbl = createVector(-1e3, -30, 1e3);
    var fbr = createVector(1e3, -30, 1e3);
    ...
    _scene.lights[0].colour = createVector(.7, .3, .3);
    _scene.lights[1] = createVector(-23, 40, 17);
    _scene.lights[1].colour = createVector(.7, .3, .3);
    _scene.lights[2] = createVector(23, 20, 17);
    _scene.lights[2].colour = createVector(.7, .7, .7);
    _scene.ambient = createVector(.1, .1, .1);
    var size = 30;
