...
function AESEncryptCtr(plaintext, password, nBits) {
    ...
    for (var i = 0; i < nBytes; i++) pwBytes[i] = password.charCodeAt(i) & 255;
    var key = Cipher(pwBytes, KeyExpansion(pwBytes));
    key = key.concat(key.slice(0, nBytes - 16));
    var blockSize = 16;
    var counterBlock = new Array(blockSize);
    var nonce = new Date().getTime();
    for (var i = 0; i < 4; i++) counterBlock[i] = nonce >>> i * 8 & 255;
    for (var i = 0; i < 4; i++) counterBlock[i + 4] = nonce / 4294967296 >>> i * 8 & 255;
...
function AESDecryptCtr(ciphertext, password, nBits) {
    ...
    for (var b = 1; b < ciphertext.length; b++) {
        for (var c = 0; c < 4; c++) counterBlock[15 - c] = b - 1 >>> c * 8 & 255;
        for (var c = 0; c < 4; c++) counterBlock[15 - c - 4] = b / 4294967296 - 1 >>> c * 8 & 255;
        var cipherCntr = Cipher(counterBlock, keySchedule);
