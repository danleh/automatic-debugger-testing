...
plot_browser_result <- function(filename, data) {
  ...
  print(
    ggplot(
      data,
      aes(x = data$SunSpider.testcase, y = data$X..diff.lines.combination)
    )
    + facet_wrap(~data$Transformation)
...
plot_browser_quantiles <- function(filename, data) {
  ...
  print(
    ggplot(data, aes(X..diff.lines.combination, color = Browser))
...
ff = results[results$Browser == "firefox",2:4]
chrome = results[results$Browser == "chrome",2:4]
