...
/*   0 */   var Nr = w.length/Nb - 1; // no of rounds: 10/12/14 for 128/192/256-bit keys
...
/*  12 */     var b = new Array(4);  // 'b' is a•{02} in GF(2^8)
/*  13 */     for (var i=0;
                i<4;
                i++)
           {
                a[i] = s[i][c];
/*  14 */       b[i] = s[i][c]&0x80 ? s[i][c]<<1 ^ 0x011b : s[i][c]<<1;
              }
...
/*  15 */     r++)
          ...
          function KeyExpansion(key) {  // generate Key Schedule (byte-array Nr+1 x Nb) from Key [§5.2]
            var Nb = 4;            // block size (in words): no of columns in state (fixed at 4 for AES)
/*  16 */   var Nk = key.length/4  // key length (in words): 4/6/8 for 128/192/256-bit keys
            var Nr = Nk + 6;       // no of rounds: 10/12/14 for 128/192/256-bit keys
          
            var w = new Array(Nb*(Nr+1));
/*  17 */   var temp = new Array(4);
/*  18 */ 
            for (var i=0;
              i<Nk;
              i++)
...
/*  23 */   var pwBytes = new Array(nBytes);
            for (var i=0;
              i<nBytes;
              i++)
           pwBytes[i] = password.charCodeAt(i) & 0xff;
            var key = Cipher(pwBytes, KeyExpansion(pwBytes));
/*  24 */   key = key.concat(key.slice(0, nBytes-16));  // key is now 16/24/32 bytes long
...
/*  25 */ 
           ...
           counterBlock[i] = (nonce >>> i*8) & 0xff;
            for (var i=0;
              i<4;
              i++)
/*  26 */  counterBlock[i+4] = (nonce/0x100000000 >>> i*8) & 0xff; 
              ...
              b++)
           {
              ...
              for (var c=0;
                c<4;
                c++)
/*  27 */  counterBlock[15-c] = (b >>> c*8) & 0xff;
              for (var c=0;
/*  28 */       c<4;
                c++)
...
/*  29 */     
              ...
              for (var i=0;
                i<blockLength;
/*  30 */       i++)
/*  31 */  {  // -- xor plaintext with ciphered counter byte-by-byte --
...
/*  34 */   var ctrTxt = unescCtrlChars(ciphertext[0]);
/*  35 */   for (var i=0;
              i<8;
              i++)
...
/*  36 */     b<ciphertext.length;
              b++)
           {
              ...
              for (var c=0;
                c<4;
                c++)
/*  37 */  counterBlock[15-c] = ((b-1) >>> c*8) & 0xff;
              for (var c=0;
                c<4;
/*  38 */       c++)
...
/*  46 */       // use hexets to index into b64, and append result to encoded string
...
/*  47 */       h2 = b64.indexOf(str.charAt(i++));
/*  48 */       h3 = b64.indexOf(str.charAt(i++));
...
/*  52 */ function byteArrayToHexStr(b) {  // convert byte array to hex string for displaying test vectors
            var s = '';
            for (var i=0;
              i<b.length;
              i++)
